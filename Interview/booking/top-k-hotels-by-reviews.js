...
const awardTopKHotels = (positiveKeywords, negativeKeywords, hotelIds, reviews, k) => {

    let [n, res ] = [reviews.length, []];
    const [posWords, negWords, hotelMap] = [positiveKeywords.split(" "), negativeKeywords.split(" "), new Map()];

    for(let i = 0; i < n; i++) {

        let hotelId = hotelIds[i];
        let review = reviews[i];
        let hotelReputation = 0;

        //calculate current review reputation
        for (const word of posWords) {

            // creating regex, global, ignorecase
            const re = new RegExp(word, 'gi');

            // matching the pattern
            const matches = review.match(re);

            // positive keyword weigh 3
            if(matches){
                hotelReputation += (matches.length * 3);
            }
        }
        ...
        for (const word of negWords) {
            ...
            // matching the pattern
            const matches = review.match(re);
            // positive keyword weigh -1
            if(matches){
                hotelReputation -= matches.length;
            }
        }

        
        if(hotelMap.has(String(hotelId))){
            hotelMap.set(String(hotelId), hotelMap.get(String(hotelId)) + hotelReputation);
        } else {
            hotelMap.set(String(hotelId), hotelReputation);
        }
    }
...
console.log(awardTopKHotels(pos, neg, hIds, rev, k)); // 2,1
// Explanation:
// Hotel 2 has a score of 21. The score is calculated as follows:

// "location" and "city center" are mentioned twice each (score = 2 * 3 + 2 * 3 = 12), while
// "breakfast", "price", and "staff" are mentioned once each (score = 12 + (1 * 3 + 1 * 3 + 1 * 3) = 21).
// Hotel 1 on the other hand has a score of 17. The score is calculated as follows:

// "location" and "city center" also twice each (score = 2 * 3 + 2 * 3 = 12)
